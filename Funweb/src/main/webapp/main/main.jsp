...
<%-- 		<%@ include file="../inc/top.jsp" %> --%>
		...
		<article id="front">
		  	...
		  	<div class="clear"></div>
			<div id="sec_news">
				<h3 onclick="location.href='../public/public.jsp'"><span class="orange">Public</span> News</h3>
				<table>
					<%
					PublicBoardDAO pbd = new PublicBoardDAO();
					ArrayList<PublicBoardBean> recentBoardList = pbd.selectRecentBoardList();
					 // PublicBoardDAO 객체의 selectRecentBoardList() 메서드 호출하여 최근 게시물 5개 조회요청
					for(PublicBoardBean pbb : recentBoardList){
					%>
					<tr onclick="location.href='../public/public_content.jsp?num=<%=pbb.getNum()%>&page=1'">
						<td width="100" class="contxt"><%=pbb.getSubject() %></td>
						<td width="80"><%=pbb.getContent() %></td> <!-- 게시글의 내용도 조회할수있게 하였다 -->
						<td width="80"><%=pbb.getName() %></td>
						<td width="80"><%=pbb.getDate()%></td>
					</tr>
					<%} %>
				</table>
			</div>
			...
			<div id="news_notice">
		  		<h3 class="brown" onclick="location.href='../center/notice.jsp'">News &amp; Notice</h3>
				<table>
					<%
