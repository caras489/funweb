...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
// 삭제 작업 하는데 필요한 것(삭제할 해당 글 번호와 페이지번호 + 패스워드 일치 여부)
int num = Integer.parseInt(request.getParameter("num"));
String pageNum = request.getParameter("page");
String pass = request.getParameter("pass");

// PublicBoardDAO 객체의 deleteBoard() 메서드를 호출하여 게시물 삭제 작업 요청
// => 파라미터 : 글번호(num), 패스워드(pass)    리턴타입 : int(deleteCount)
// => 패스워드를 조회하여 패스워드가 일치할 경우에만 레코드 삭제
PublicBoardDAO pbd = new PublicBoardDAO();
int deleteCount = pbd.deleteBoard(num, pass);

// 작업 작업 결과 판별 및 처리
// deleteCount > 0 일 경우 "public.jsp" 페이지로 이동(페이지번호 전달)하고
